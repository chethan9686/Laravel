...
use App\Imports\DelhiAttendenceImport;
...
class AttendenceDelhiController extends Controller
{
    ...
    {      
        return view('user.attendence.delhi.index');
    }   
    ...
    public function attendencelist()
    {
        ...
        {           
            $this->calculate_del_attendence($data->id);
        }

        $users = DelhiAttendence::where('year','=',$today->year)->where('month','=',$today->month)->distinct()->select($dates)->orderBy('emp_name','ASC')->get();

        return view('user.attendence.delhi.attendence_list',compact('users','calendar','Current','LastMonth'));
    }
    ...
    {
        $Attendence = DelhiAttendence::find($request->id);
    ...
    {  
        if($request->hasFile('attendence_file'))
        {
            ...
            {         	
                ...
                Excel::import(new DelhiAttendenceImport, $file);

                $Latest = DelhiPunchIn::orderBy('id', 'DESC')->first();
                ...
                $users = DelhiAttendence::where('year','=',$date->year)->where('month','=',$date->month)->distinct()->get(); 

                foreach ($users as $key => $user) 
                {
                    $punch_in = DelhiPunchIn::where('date','=',$date->format('Y-m-d'))->where('emp_id','=',$user->emp_id)->first();
                    ...
                    {               
                        ...
                        {              
                            ...
                            {      
                                ...
                                {
                                    if($punch_in->time < '12:00:00')
                                    {
                                        $user->$num_word = 'P'; 
                                        $user->save();
                                    }
                                    elseif($punch_in->time > '12:00:00' && $punch_in->time < '14:30:00')
                                    {
                                        $user->$num_word = 'NHD';  
                                        $user->save();   
                                    ...
                                    elseif($punch_in->time > '14:30:00')
                                    {
                                        $user->$num_word = 'AB';    
                                        $user->save(); 
                                        Mail::to($User)->send(new UserSecondHalfComing($User,$punch_in,$cc,"DL"));
                                    }                           
                                }
    ...
    public function calculate_del_attendence($data)
    {        
    ...
    public function delhi_mail_attendence()
    {
        $today = Carbon::now();
        $users = DelhiAttendence::where('year','=',$today->year)->where('month','=',$today->month)->distinct()->orderBy('emp_name','ASC')->get(); 

        $day = $today->day;

        $number = new ConvertNumber(); 
        $num_word = $number->numberTowords($day);  
        $absent_users = DelhiAttendence::where('year','=',$today->year)->where('month','=',$today->month)->where($num_word,'=','AB')->distinct()->orderBy('emp_name','ASC')->get(); 
       
        return view('user.attendence.delhi.mail_attendence',compact('users','absent_users'));
    }

    public function delhi_send_mail(Request $request)
    {
        $request->validate([
                'check_user' => ['required'],               
                'mail_date' => ['required'], 
