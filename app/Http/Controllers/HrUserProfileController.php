...
{
    public function showUserProfile($EncryptedUserID)
    {
        ...
        $official_documents = UserOfficialDocument::where('user_id',$user->id)->first();  

        $kra = current(explode('|',$official_documents->kra)); 
        $commitment = current(explode('|',$official_documents->commitment));
        $offer_letter = current(explode('|',$official_documents->offer_letter));   
        $yearly_appraisal = current(explode('|',$official_documents->yearly_appraisal)); 
    ...
    {         
        ...
        {                
            $Attendence = BangaloreAttendence::where('user_id','=',$UserID)->get();
        ...
        elseif($user->branch == 2 || $user->branch == 5)
        {
            $Attendence = MumbaiAttendence::where('user_id','=',$UserID)->get();
            foreach($Attendence as $Attd)
            {
                $Attd->emp_id = $request->emp_id; 
                $Attd->save();
        ...
        elseif($user->branch == 3)
        {               
            $Attendence = DelhiAttendence::where('user_id','=',$UserID)->get();
            foreach($Attendence as $Attd)
            {
                $Attd->emp_id = $request->emp_id; 
                $Attd->save();
   ...
   public function upload_personal_documents(Request $request)
    ...
    public function user_references(Request $request)
    {
     ...
     public function upload_official_documents(Request $request)
    {
        $UserID = Crypt::decrypt($request->user_id);
        ...
        $request->validate([
                'kra' => ['max:10240'],
                'commitment' => ['max:10240'],
                'offer_letter' => ['max:10240'],
                'yearly_appraisal' => ['max:10240'],               
        ], [
                'kra.max' => 'Maximum file size to upload is 10MB',
                'commitment.max' => 'Maximum file size to upload is 10MB',
                'offer_letter.max' => 'Maximum file size to upload is 10MB',
                'yearly_appraisal.max' => 'Maximum file size to upload is 10MB',                        
        ]);       
      
        if($request->hasfile('kra'))
         {           
            foreach($request->file('kra') as $file)
            {
                $name = rand().$time.'.'.$file->extension();
                $file->move(public_path().'/user/kra/'.$user->id.'/', $name);
                $data[] = 'user/kra/'.$user->id.'/'. $name;      
            }

            $Official_Document->kra=implode($data,'|');
            $Official_Document->save();
         }      
         
        if($request->hasfile('commitment'))
         {           
            foreach($request->file('commitment') as $file)
            {
                $name = rand().$time.'.'.$file->extension();
                $file->move(public_path().'/user/commitment/'.$user->id.'/', $name);
                $commitmentdata[] = 'user/commitment/'.$user->id.'/'. $name;      
            }

            $Official_Document->commitment=implode($commitmentdata,'|');
            $Official_Document->save();
         }       
         ...
         {           
            ...
            {
                $offer_letter = rand().$time.'.'.$file->extension();
                $file->move(public_path().'/user/offer_letter.'.$user->id.'/', $offer_letter);
                $offer_letterdata[] = 'user/offer_letter/'.$user->id.'/'. $offer_letter;      
            }

            $Official_Document->offer_letter=implode($offer_letterdata,'|');
            $Official_Document->save();
         }    

         if($request->hasfile('yearly_appraisal'))
         {           
            foreach($request->file('yearly_appraisal') as $file)
            {
                $yearly_appraisal = rand().$time.'.'.$file->extension();
                $file->move(public_path().'/user/yearly_appraisal/'.$user->id.'/', $yearly_appraisal);
                $yearly_appraisaldata[] = 'user/yearly_appraisal/'.$user->id.'/'.$yearly_appraisal;      
            }

            $Official_Document->yearly_appraisal=implode($yearly_appraisaldata,'|');
            $Official_Document->save();
         }    

        Toastr::success('You Have Successfully Updated Official Documents!. Thank You ',$title = null, $options = ["positionClass" => "toast-top-center"]);
        return back();
    ...
    {   
        ...
        switch ($Document) {
            ...
            break;     
            case 'Offer_Letter':
                    $fileName = rand().'Offer_Letter.zip';   
                    $fileName = '/Documents/'.$fileName;   
                    $Offer_Letters = $Official_Documents->select('Offer_Letter')->first();
                    $offer_letter = explode('|',$Offer_Letters->Offer_Letter);  

                   if ($zip->open(public_path($fileName), ZipArchive::CREATE) === TRUE)
                    {
                    foreach($offer_letter as $letter){                      
                    ...
                    return response()->download(public_path($fileName));     
            break; 
            case 'Yearly_Appraisal':
                    $fileName = rand().'Yearly_Appraisal.zip';   
                    $fileName = '/Documents/'.$fileName;   
                    $Yearly_Appraisals = $Official_Documents->select('Yearly_Appraisal')->first();
                    $yearly_appraisal = explode('|',$Yearly_Appraisals->Yearly_Appraisal);  

                   if ($zip->open(public_path($fileName), ZipArchive::CREATE) === TRUE)
                    {
                    foreach($yearly_appraisal as $appraisal){                      
