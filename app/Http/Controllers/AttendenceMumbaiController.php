...
class AttendenceMumbaiController extends Controller
{
    ...
    {      
        return view('user.attendence.mumbai.index');
    }   
    ...
    public function attendencelist()
    {
        ...
        {           
            $this->calculate_mum_attendence($data->id);
        }

        $users = MumbaiAttendence::where('year','=',$today->year)->where('month','=',$today->month)->distinct()->select($dates)->orderBy('emp_name','ASC')->get();

        return view('user.attendence.mumbai.attendence_list',compact('users','calendar','Current','LastMonth'));
    }
                            ...
                            {      
                                ...
                                elseif (!is_null($Lateworking)) 
                                {
                                    ...
                                    elseif($punch_in->time > '14:30:00')
                                    {
                                        $user->$num_word = 'AB';    
                                        $user->save(); 
                                        Mail::to($User)->send(new UserSecondHalfComing($User,$punch_in,$cc,"MB")); 
                                    }                           
                                }
    ...
    public function calculate_mum_attendence($data)
    {        
    ...
    public function mumbai_mail_attendence()
    {
        $today = Carbon::now();
        $users = MumbaiAttendence::where('year','=',$today->year)->where('month','=',$today->month)->distinct()->orderBy('emp_name','ASC')->get(); 

        $day = $today->day;

        $number = new ConvertNumber(); 
        $num_word = $number->numberTowords($day);  
        $absent_users = MumbaiAttendence::where('year','=',$today->year)->where('month','=',$today->month)->where($num_word,'=','AB')->distinct()->orderBy('emp_name','ASC')->get(); 
       
        return view('user.attendence.mumbai.mail_attendence',compact('users','absent_users'));
    }

    public function mumbai_send_mail(Request $request)
    {
        $request->validate([
                'check_user' => ['required'],               
                'mail_date' => ['required'], 
