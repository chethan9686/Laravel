...
<title>HRM | View Appriciation</title>
...
<link  rel="stylesheet" href="{{asset('admin/lib/css/emoji.css')}}"/>
            ...
            <div class="row">
                <div class="col-lg-7 col-md-6 col-sm-12">
                    ...
                    <ul class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">New Appreciation</li>
                    </ul>
         ...
         <div class="row">
            <div class="col-lg-8 col-md-12">
                <div class="card">
                  <div class="header">
                        @php
                          $eventdate = \Carbon\Carbon::parse($viewappriciation->eventdate);    
                          $eventdate = $eventdate->format('l jS F Y'); 
                        @endphp
                        <h2><strong>{{ $viewappriciation->eventname }}</strong></h2> 
                        <span class="blogitem-date">{{ $eventdate}}</span>                       
                    </div>
                    <blockquote class="blockquote">
                        <p>{!! $viewappriciation->clientmail !!}</p>
                        <footer>by <a href="#">{{ $viewappriciation->clientname }}</a></footer>
                    </blockquote>
                   <div class="chat_window body">
                        <div class="chat-header">
                            <div class="user">
                                <div class="chat-about">
                                    <div class="chat-with">{{ $viewappriciation->eventname }} - 2019</div>
                                    <div class="chat-num-messages">{{ $viewappriciation->clientname }}</div>
                                </div>
                            </div>                              
                        ...
                        <ul class="chat-history">
                          @php
                            $latestwish = \App\Appriciationwish::where('appriciationmail_id','=',$viewappriciation->id)->get();                      
                          @endphp
                        ...
                        <form method="POST" action="{{ route('admin.appriciation_wish') }}" id="appriciationwishesh">
                          {{ csrf_field() }}                       
                          <input type="hidden" name="appriciationmail_id" value="{{$viewappriciation->id}}">
                          <div class="text-left">
                            <div class="lead emoji-picker-container">
                              <div class="chat-box">
                                <div class="input-group sendbutton">
                                    <input type="text" name="appriciation_whishes" class="form-control" placeholder="Enter text here..." data-emojiable="true" style="margin-top: 20px !important;" required>
            ...
            <div class="col-lg-4 col-md-12">                  
                <div class="card">
                    ...
                    <div class="body">
                        ...
                        @forelse($maildata as $data)
                          @php
                            $EncryptedId = \Illuminate\Support\Facades\Crypt::encrypt($data->id);
                          @endphp
                            <li><a href="{{ route('admin.view_appriciation',[ 'EncryptedId' => $EncryptedId ])}}">{{ $data->eventname}}</a></li>
                        @empty
                          <li>No Data</li>
                        @endforelse                       
                        </ul>
...
<script src="{{asset('admin/lib/js/config.js')}}"></script>
<script src="{{asset('admin/lib/js/util.js')}}"></script>
<script src="{{asset('admin/lib/js/jquery.emojiarea.js')}}"></script>
<script src="{{asset('admin/lib/js/emoji-picker.js')}}"></script>
  <script>
      $(function() {

        window.emojiPicker = new EmojiPicker({
          emojiable_selector: '[data-emojiable=true]',
          assetsPath: '/admin/lib/img/',
